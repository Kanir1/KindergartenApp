    ...
    try {
      const payload = {
        ...
        newChild: {
          ...
          externalId: data.childExternalId.trim(),
      ...
      nav('/parent'); // go to parent dashboard
    } catch (err) {
    ...
    <div className="max-w-md mx-auto p-4 space-y-4">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-3">
        <div>
          ...
          <input className="border p-2 w-full" {...register('name')} placeholder="Full name" />
          {errors.name && <p className="text-red-600 text-sm">{errors.name.message}</p>}
        </div>

        <div>
          <label className="block text-sm">Email</label>
          <input className="border p-2 w-full" {...register('email')} placeholder="you@example.com" />
        ...
        <div>
          ...
          <input className="border p-2 w-full" {...register('childName')} placeholder="e.g., Tommy" />
